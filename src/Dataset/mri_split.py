...
def tumor_ratio_bins(data: Patients) -> tuple[list[int], list[float]]:
    bins = [] # Current bins [ 0-20%, 21-40%, 41-100% ]
    # Iterate through patients (Multithreading)
    with ThreadPoolExecutor() as exe:
        ratios = list(exe.map(patient_tumor_ratio, data.values()))
...
def split_patients(data: Patients, seed: int = 42) -> tuple[Patients, Patients, Patients]:
    ...
    test_ratios = [key_ratio_dict[key] for key in test_keys]
    ...
    valid_ratios = [key_ratio_dict[key] for key in valid_keys]
    train_ratios = [key_ratio_dict[key] for key in train_keys]
